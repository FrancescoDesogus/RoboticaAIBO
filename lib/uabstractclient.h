...
/** Container for URBI values in lists
 * Only simple types (no list, no binary) are allowed for now
 */
class UValue {
 public:
...
/**
 * Destination buffer is reallocated if necessary: it must be valid en destination.size must be its size,
 * or both must be 0.
*/
...
namespace urbi {
  ...
  /// Send a possibly armored string to the default client (for internal use)
